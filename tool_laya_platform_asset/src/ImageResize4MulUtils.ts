...
export class ImageResize4MulUtils {
    ...
    static async ImageResizeSync(srcPath: string, destPath: string, size: string, isOver = true, quality: number) {
        ...
        var scale = parseFloat(size.replace('%', '')) / 100;

        var imgInfo = imagesize(srcPath);
        var w = imgInfo.width as number;
        var h = imgInfo.height as number;
        w = Math.ceil(w * scale);
        h = Math.ceil(h * scale);
        w = Math.floor(w / 4) * 4;
        h = Math.floor(h / 4) * 4;
        ...
        console.log(imgInfo.width, imgInfo.height, scale, w, h);
        size = `${w}x${h}!`;
